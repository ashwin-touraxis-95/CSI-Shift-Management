...
export default function Display() {
  ...
  // Colors — all with safe dark defaults independent of sidebar theme
  const primary = theme.primary_color || '#C0392B';
  const bg = theme.display_bg || '#0D1117';
  const textColor = theme.display_text || '#FFFFFF';
  const mutedColor = theme.display_muted || 'rgba(255,255,255,0.4)';
  const cardBg = theme.display_card_bg || 'rgba(255,255,255,0.04)';
  const borderColor = theme.display_border || 'rgba(255,255,255,0.08)';
  const headerBg = theme.display_header_bg || 'rgba(255,255,255,0.03)';
    ...
    return (
      <div style={{ minHeight:'100vh', background:bg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', fontFamily:'inherit' }}>
        <div style={{ textAlign:'center', marginBottom:40 }}>
          ...
          <h1 style={{ color:textColor, fontSize:28, fontWeight:800, margin:'0 0 6px' }}>{theme.company_name || 'ShiftManager'}</h1>
          <p style={{ color:mutedColor, fontSize:15, margin:0 }}>Display Screen</p>
        </div>
        <form onSubmit={handlePin} style={{ background:cardBg, border:`1px solid ${borderColor}`, borderRadius:16, padding:40, width:320, textAlign:'center' }}>
          <p style={{ color:mutedColor, fontSize:14, marginTop:0, marginBottom:20 }}>Enter display PIN to continue</p>
          <input ref={pinRef} type="password" inputMode="numeric" maxLength={8} value={pinInput}
            ...
            style={{ width:'100%', padding:'14px', borderRadius:10, border:`1.5px solid ${borderColor}`, background:cardBg, color:textColor, fontSize:22, textAlign:'center', letterSpacing:8, outline:'none', fontFamily:'inherit', marginBottom:16, boxSizing:'border-box' }}/>
  ...
  const AgentCard = ({ agent, statusColor }) => (
    <div style={{ padding:'14px 16px', borderRadius:12, marginBottom:10, border:`1px solid ${borderColor}`, borderLeft:`3px solid ${statusColor}`, background:cardBg, display:'flex', alignItems:'center', gap:14 }}>
      ...
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ fontWeight:700, fontSize:16, color:textColor, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{agent.name}</div>
        <div style={{ fontSize:12, marginTop:3 }}>
          {agent.status === 'on_break'
            ? <span style={{ color:'#F59E0B', fontWeight:600 }}>{agent.break_type_icon} {agent.break_type_name}</span>
            : <span style={{ color:mutedColor }}>{agent.department}</span>}
        </div>
      </div>
    </div>
  );
  ...
  const ColHeader = ({ label, count, color, pulse }) => (
    <div style={{ padding:'16px 20px', borderBottom:`1px solid ${borderColor}`, display:'flex', alignItems:'center', gap:10 }}>
      ...
      <div style={{ fontSize:12, fontWeight:700, textTransform:'uppercase', letterSpacing:1.5, color:mutedColor }}>{label}</div>
      <div style={{ marginLeft:'auto', fontSize:12, fontWeight:700, fontFamily:'monospace', background:cardBg, padding:'2px 10px', borderRadius:10, color:mutedColor }}>{count}</div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight:'100vh', background:bg, display:'flex', flexDirection:'column', fontFamily:'inherit' }}>
      ...
      {/* Top bar */}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 32px', background:headerBg, borderBottom:`1px solid ${borderColor}`, flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:14 }}>
          ...
          <div>
            <div style={{ fontWeight:700, fontSize:20, color:textColor }}>{theme.company_name || 'ShiftManager'}</div>
            <div style={{ fontSize:12, color:mutedColor, marginTop:2 }}>Live Availability</div>
          </div>
        </div>
        <div style={{ textAlign:'right' }}>
          <div style={{ fontFamily:'monospace', fontSize:38, fontWeight:500, color:textColor, letterSpacing:2, lineHeight:1 }}>
      ...
      <div style={{ flex:1, display:'grid', gridTemplateColumns:'1fr 1fr 1fr', overflow:'hidden' }}>
        <div style={{ borderRight:`1px solid ${borderColor}`, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          <div style={{ flex:1, padding:'12px 16px', overflowY:'auto' }}>
            ...
            {online.length===0 && <div style={{ padding:40, textAlign:'center', color:mutedColor, fontSize:13 }}>No one online</div>}
          </div>
        </div>
        <div style={{ borderRight:`1px solid ${borderColor}`, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          <div style={{ flex:1, padding:'12px 16px', overflowY:'auto' }}>
            ...
            {onBreak.length===0 && <div style={{ padding:40, textAlign:'center', color:mutedColor, fontSize:13 }}>No one away</div>}
          </div>
        </div>
        <div style={{ display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          <div style={{ flex:1, padding:'12px 16px', overflowY:'auto' }}>
            ...
            {offline.length===0 && <div style={{ padding:40, textAlign:'center', color:mutedColor, fontSize:13 }}>No one offline</div>}
          </div>
        </div>
      </div>

      {/* Footer */}
      <div style={{ padding:'10px 32px', background:headerBg, borderTop:`1px solid ${borderColor}`, display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, color:mutedColor }}>
          <div style={{ width:6, height:6, borderRadius:'50%', background:'#22C55E', animation:'pulse 1.5s infinite' }}/>
          Live · Updates automatically
        </div>
        <div style={{ fontSize:11, color:mutedColor }}>{theme.company_name||'ShiftManager'} · Built by Ashwin Halford</div>
      </div>
    </div>
  );
}
