  ...
  return (
    <>
      ...
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:16, alignItems:'center' }}>
        {[
          { label:'All Departments', value:'all', options:[{value:'all',label:'All Departments'},...depts.map(d=>({value:d,label:d}))], state:filterDept, set:setFilterDept },
          { label:'All Locations',   value:'all', options:[{value:'all',label:'All Locations'},...locations.map(l=>({value:l.code,label:`${l.name} (${l.code})`}))], state:filterLocation, set:setFilterLocation },
          { label:'All User Types',  value:'all', options:[{value:'all',label:'All User Types'},{value:'account_admin',label:'Account Admin'},{value:'manager',label:'Manager'},{value:'team_leader',label:'Team Leader'},{value:'agent',label:'Agent'}], state:filterUserType, set:setFilterUserType },
          { label:'All Job Roles',   value:'all', options:[{value:'all',label:'All Job Roles'},...jobRoles.map(jr=>({value:jr.id,label:jr.name}))], state:filterJobRole, set:setFilterJobRole },
        ].map(f => (
          <select key={f.label} value={f.state} onChange={e=>f.set(e.target.value)}
            style={{ padding:'7px 12px', borderRadius:8, border:`1.5px solid ${f.state!=='all'?'var(--red)':'var(--gray-200)'}`, fontFamily:'inherit', fontSize:13, background:f.state!=='all'?'#fef2f2':'white', color:f.state!=='all'?'var(--red)':'var(--gray-700)', cursor:'pointer', fontWeight:f.state!=='all'?700:400 }}>
        ...
        {(filterDept!=='all'||filterLocation!=='all'||filterUserType!=='all'||filterJobRole!=='all') && (
          <button onClick={()=>{setFilterDept('all');setFilterLocation('all');setFilterUserType('all');setFilterJobRole('all');}}
            style={{ padding:'7px 12px', borderRadius:8, border:'1.5px solid var(--gray-200)', background:'white', fontSize:13, fontFamily:'inherit', cursor:'pointer', color:'var(--gray-500)' }}>✕ Clear</button>
        )}
        <span style={{ marginLeft:'auto', fontSize:12, color:'var(--gray-400)' }}>{displayed.length} user{displayed.length!==1?'s':''}</span>
      </div>
