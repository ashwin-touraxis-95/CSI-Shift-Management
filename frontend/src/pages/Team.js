...
export default function Team() {
  ...
  return (
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      ...
      {message.text && <div style={{ background:message.type==='error'?'#fef2f2':'#d4edda', border:`1px solid ${message.type==='error'?'#fca5a5':'#c3e6cb'}`, borderRadius:8, padding:'10px 16px', marginBottom:12, color:message.type==='error'?'#dc2626':'#155724', fontSize:14 }}>{message.text}</div>}

      {/* Role guide */}
      {(isAdmin||isManager) && (
        <div style={{ background:'#f0f9ff',border:'1px solid #bae6fd',borderRadius:10,padding:'14px 20px',marginBottom:12 }}>
      ...
      <div style={{ display:'flex', flexDirection:'row', gap:8, flexWrap:'nowrap', marginBottom:0, alignItems:'center' }}>
        {[
          { label:'All Departments', options:[{value:'all',label:'All Departments'},...depts.map(d=>({value:d,label:d}))], state:filterDept, set:setFilterDept },
          { label:'All Locations',   options:[{value:'all',label:'All Locations'},...locations.map(l=>({value:l.code,label:`${l.name} (${l.code})`}))], state:filterLocation, set:setFilterLocation },
          { label:'All User Types',  options:[{value:'all',label:'All User Types'},{value:'account_admin',label:'Account Admin'},{value:'manager',label:'Manager'},{value:'team_leader',label:'Team Leader'},{value:'agent',label:'Agent'}], state:filterUserType, set:setFilterUserType },
          { label:'All Job Roles',   options:[{value:'all',label:'All Job Roles'},...jobRoles.map(jr=>({value:jr.id,label:jr.name}))], state:filterJobRole, set:setFilterJobRole },
        ].map(f => {
          const active = f.state !== 'all';
          return (
            <select key={f.label} value={f.state} onChange={e=>f.set(e.target.value)}
              style={{ flex:'1 1 0', minWidth:0, padding:'7px 10px', borderRadius:8,
                ...
                color: active?'var(--red)':'var(--gray-700)',
        ...
        {(filterDept!=='all'||filterLocation!=='all'||filterUserType!=='all'||filterJobRole!=='all') && (
          <button onClick={()=>{setFilterDept('all');setFilterLocation('all');setFilterUserType('all');setFilterJobRole('all');}}
            style={{ padding:'7px 10px', borderRadius:8, border:'1.5px solid var(--gray-200)', background:'white', fontSize:12, fontFamily:'inherit', cursor:'pointer', color:'var(--gray-500)', whiteSpace:'nowrap', flexShrink:0 }}>✕ Clear</button>
        )}
        <span style={{ fontSize:12, color:'var(--gray-400)', whiteSpace:'nowrap', flexShrink:0 }}>{displayed.length} user{displayed.length!==1?'s':''}</span>
      </div>
                  ...
                  <td style={{ padding:'12px 16px' }}>
                    {isEditing
                      ? <select value={cur.location||'SA'} onChange={e => {
                          ...
                          {cur.user_type === 'team_leader' && <option value="all">🌍 All Locations</option>}
                          {locations.length > 0
                            ? locations.map(l => <option key={l.code} value={l.code}>{l.name} ({l.code})</option>)
                            : <><option value="SA">South Africa (SA)</option><option value="PH">Philippines (PH)</option></>
                          }
                        </select>
                      : (() => { const locObj = locations.find(l=>l.code===(user.location||'SA')); const isAll=user.location==='all'; const isSA=(user.location||'SA')==='SA'; return <span style={{ padding:'3px 10px', borderRadius:20, fontSize:12, fontWeight:700, background:isAll?'#f3f4f6':isSA?'#ECFDF5':'#EFF6FF', color:isAll?'#374151':isSA?'#065F46':'#1D4ED8' }}>{isAll?'🌍 All Locations':locObj?.name||user.location||'SA'}</span>; })()
                    }
                  </td>
                  <td style={{ padding:'12px 16px' }}>
    ...
    {/* Temp Password Modal - outside scroll so it overlays correctly */}
      {tempPasswordModal && (
