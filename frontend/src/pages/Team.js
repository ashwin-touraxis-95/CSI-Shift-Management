...
export default function Team() {
  ...
  const fetchDepts = async () => {
    try { const r = await axios.get('/api/settings/departments'); setDepts(Array.isArray(r.data) ? r.data.map(d=>d.name) : []); }
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (r.data.temp_password) {
        alert(`✅ User created!\n\n🔑 Temporary Password:\n\n${r.data.temp_password}\n\nShare this with ${newUser.name} — they will be asked to create a new password on their first login.`);
      }
      setShowAdd(false);
  ...
  const handleSave = async (user) => {
    try {
      await axios.put(`/api/users/${user.id}`, { user_type:user.user_type, department:user.department, name:user.name, timezone:user.timezone||'Africa/Johannesburg' });
  ...
  const handleResetPassword = async (user) => {
    ...
    try {
      ...
      alert(`🔑 Password reset for ${user.name}\n\nTemporary Password:\n\n${r.data.temp_password}\n\nShare this with them — they will be forced to create a new password on their next login.`);
