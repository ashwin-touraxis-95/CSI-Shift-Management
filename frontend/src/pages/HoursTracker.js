...
function DownloadScopeButtons({ downloadCSV, userLocation, isLeader, isAdmin, isManager }) {
  const defaultScope = userLocation === 'all' ? 'all' : (userLocation || 'SA');
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center' }}>
          ...
          {(isAdmin || isManager || isLeader) && (
            <DownloadScopeButtons downloadCSV={downloadCSV} userLocation={user?.location||'SA'} isLeader={isLeader} isAdmin={isAdmin} isManager={isManager} />
          )}
        </div>
      </div>
