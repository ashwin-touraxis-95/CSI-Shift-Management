      ...
      <div style={{ display:'grid', gridTemplateColumns:showClockPanel ? '1fr 320px' : '1fr', gap:24, alignItems:'start' }}>
        ...
        {showClockPanel && (
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            {(() => {
              const today = new Date().toISOString().split('T')[0];
              const todayShifts = shifts.filter(s => s.user_id === user?.id && s.date === today && s.status !== 'draft');
              if (todayShifts.length === 0) return null;
              return (
                <div className="card" style={{ padding:24 }}>
                  <h3 style={{ margin:'0 0 14px', fontWeight:700, fontSize:16 }}>📅 Today's Shift</h3>
                  {todayShifts.map(s => (
                    <div key={s.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 14px', borderRadius:10, background:'#F8FAFC', border:'1px solid #E2E8F0' }}>
                      <div>
                        <div style={{ fontWeight:700, fontSize:15, fontFamily:'DM Mono', color:primary }}>{s.start_time} – {s.end_time}</div>
                        <div style={{ fontSize:12, color:'#9CA3AF', marginTop:2 }}>{s.department}{s.notes ? ` · ${s.notes}` : ''}</div>
                      </div>
                    </div>
                  ))}
                </div>
