...
export default function Dashboard() {
  ...
  const [scheduleFilter, setScheduleFilter] = useState('mine');
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [av, bt, sh, dept] = await Promise.all([
        axios.get('/api/availability').catch(e => { console.error('availability failed:', e.message); return { data: [] }; }),
        axios.get('/api/break-types').catch(e => { console.error('break-types failed:', e.message); return { data: [] }; }),
        axios.get('/api/shifts').catch(e => { console.error('shifts failed:', e.message); return { data: [] }; }),
        axios.get('/api/departments').catch(e => { console.error('departments failed:', e.message); return { data: [] }; }),
      ]);
      console.log('break-types fetched:', bt.data?.length, bt.data);
  ...
  return (
        ...
        <div>
          ...
          {isAgent && (
            <div style={{ display:'flex', gap:8, marginBottom:16 }}>
              {[{id:'live', label:'⚡ Live'}, {id:'schedule', label:'📅 My Schedule'}].map(t => (
                <button key={t.id} onClick={() => { setTeamView(t.id === 'schedule'); setScheduleFilter('mine'); }}
                  style={{ padding:'7px 18px', borderRadius:8, border:'none', fontWeight:600, fontSize:13, cursor:'pointer', fontFamily:'inherit',
                    background: (t.id === 'live' && !teamView) || (t.id === 'schedule' && teamView) ? primary : '#F1F5F9',
                    color: (t.id === 'live' && !teamView) || (t.id === 'schedule' && teamView) ? 'white' : '#6B7280' }}>
                  {t.label}
                </button>
              ))}
            </div>
          ...
          {/* My Schedule tab for agents */}
          {isAgent && teamView && (() => {
            const today = new Date().toISOString().split('T')[0];
            const myShifts = shifts.filter(s => s.status !== 'draft');
            return (
              <div>
                <div style={{ display:'flex', justifyContent:'flex-end', marginBottom:12, gap:6 }}>
                  {[{id:'mine', label:'My Shifts'}, {id:'team', label:'Team Shifts'}].map(t => (
                    <button key={t.id} onClick={() => setScheduleFilter(t.id)}
                      style={{ padding:'5px 14px', borderRadius:8, border:'1px solid #E2E8F0', fontWeight:600, fontSize:12, cursor:'pointer', fontFamily:'inherit',
                        background: scheduleFilter === t.id ? primary : 'white',
                        color: scheduleFilter === t.id ? 'white' : '#6B7280' }}>
                      {t.label}
                    </button>
                  ))}
                </div>
                  ...
                  {scheduleFilter === 'team' ? (
                    <div>
                      <div style={{ padding:'14px 20px', borderBottom:'1px solid #F1F5F9', fontWeight:700, fontSize:15 }}>
                        {user?.department} — Team Schedule
                      </div>
                      {availability.filter(a => a.department === user?.department).length === 0
                        ? <div style={{ padding:40, textAlign:'center', color:'#9CA3AF' }}>No team members found</div>
                        : availability.filter(a => a.department === user?.department).map(a => {
                            const agentShifts = shifts.filter(s => s.user_id === a.id && s.status !== 'draft');
                            if (!agentShifts.length) return null;
                            return (
                              <div key={a.id} style={{ borderBottom:'1px solid #F8FAFC' }}>
                                <div style={{ padding:'10px 20px 4px', fontSize:12, fontWeight:700, color:'#374151' }}>{a.name}</div>
                                {agentShifts.map(s => (
                                  <div key={s.id} style={{ display:'flex', alignItems:'center', gap:14, padding:'8px 20px 8px 32px', borderBottom:'1px solid #F8FAFC' }}>
                                    <div style={{ minWidth:100, fontSize:11, fontWeight:700, color: s.date === today ? primary : '#9CA3AF', textTransform:'uppercase' }}>
                                      {s.date === today ? '⚡ Today' : new Date(s.date+'T00:00:00').toLocaleDateString('en-ZA', { weekday:'short', month:'short', day:'numeric' })}
                                    </div>
                                    <div style={{ fontSize:13, fontFamily:'DM Mono', fontWeight:700 }}>{s.start_time?.slice(0,5)} – {s.end_time?.slice(0,5)}</div>
                                    {s.notes && <div style={{ fontSize:12, color:'#9CA3AF' }}>{s.notes}</div>}
                                  </div>
                                ))}
                              </div>
                            );
                    ...
                    myShifts.length === 0
                      ? <div style={{ padding:40, textAlign:'center', color:'#9CA3AF' }}>No upcoming shifts scheduled</div>
                      : <div style={{ padding:'8px 0' }}>
                          {myShifts.map(s => (
                            <div key={s.id} style={{ display:'flex', alignItems:'center', gap:14, padding:'12px 20px', borderBottom:'1px solid #F8FAFC' }}>
                              <div style={{ minWidth:90 }}>
                                <div style={{ fontSize:11, fontWeight:700, color: s.date === today ? primary : '#9CA3AF', textTransform:'uppercase' }}>
                                  {s.date === today ? '⚡ Today' : new Date(s.date+'T00:00:00').toLocaleDateString('en-ZA', { weekday:'short', month:'short', day:'numeric' })}
                                </div>
                                <div style={{ fontSize:13, fontFamily:'DM Mono', fontWeight:700, color:'#374151', marginTop:2 }}>{s.start_time?.slice(0,5)} – {s.end_time?.slice(0,5)}</div>
                              </div>
                              <span style={{ padding:'2px 10px', borderRadius:20, fontSize:11, fontWeight:600, background:'#F1F5F9', color:'#6B7280' }}>{s.department}</span>
                              {s.notes && <div style={{ fontSize:12, color:'#9CA3AF' }}>{s.notes}</div>}
                            </div>
                          ))}
                        </div>
          ...
          {/* Live availability - always for non-agents, Live tab for agents */}
          {(!isAgent || !teamView) && (
            <div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))', gap:12, marginBottom:20 }}>
                {[
                  { label:'Online', value:online.length, color:theme?.online_color||'#22C55E' },
                  { label:'On Break', value:onBreak.length, color:'#F59E0B' },
                  { label:'Offline', value:offline.length, color:theme?.offline_color||'#94A3B8' },
                ].map(s => (
    ...
    {/* Override popup rendered at top level to avoid clipping */}
    {openOverride && (() => {
