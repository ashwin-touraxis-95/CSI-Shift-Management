...
export default function Dashboard() {
  ...
  // Close override popup on outside click
  useEffect(() => {
    const handle = (e) => { if (overrideRef.current && !overrideRef.current.contains(e.target)) setOpenOverride(null); };
  ...
  const overrideAction = async (endpoint, body, successMsg) => {
    try { await axios.post(endpoint, body); fetchAll(); showMsg(successMsg); setOpenOverride(null); }
    catch (e) { showMsg(e.response?.data?.error || 'Error', 'error'); }
  };
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 20px', borderBottom:'1px solid #F9FAFB', background: isBreaking ? `${agent.break_type_color}08` : 'white' }}>
        ...
        {canOverride && (
          <div style={{ position:'relative' }}>
            <button onClick={() => setOpenOverride(openOverride === agent.id ? null : agent.id)}
