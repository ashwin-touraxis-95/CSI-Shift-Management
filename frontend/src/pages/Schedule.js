...
export default function Schedule() {
  ...
  const [editCell, setEditCell] = useState(null); // { agent, date, shift|null }
  ...
  const openEditCell = (agent, date, shift) => {
    ...
    setEditForm({ start_time: shift?.start_time||'07:00', end_time: shift?.end_time||'15:00' });
  };
  ...
  const handleEditSave = async () => {
    ...
    try {
      if (editCell.shift) {
        await axios.put(`/api/shifts/${editCell.shift.id}`, { ...editCell.shift, start_time: editForm.start_time, end_time: editForm.end_time, status:'published' });
      } else {
        await axios.post('/api/shifts', { user_id: editCell.agent.id, date: editCell.date, start_time: editForm.start_time, end_time: editForm.end_time, status:'published' });
      }
      await fetchData();
      setEditCell(null);
                  ...
                  <tr key={agent.id} style={{ background:i%2===0?'white':'var(--gray-50)', borderBottom:'1px solid var(--gray-100)' }}>
                    ...
                    {days.map(d => {
                      ...
                      return <td key={dayStr} onClick={() => openEditCell(agent, dayStr, shift)}
                        style={{ padding:'4px 4px', verticalAlign:'top', background:isToday(d)?'rgba(192,57,43,0.04)':undefined, cursor:editable?'pointer':undefined, position:'relative' }}>
                        {dayShifts.map(s=><ShiftPill key={s.id} shift={s}/>)}
                        {editable && !shift && <div style={{ width:'100%', height:28, borderRadius:6, border:'1.5px dashed #e2e8f0', display:'flex', alignItems:'center', justifyContent:'center', color:'#cbd5e1', fontSize:11 }}>+</div>}
                      </td>;
                    })}
                  </tr>
                ))
    ...
    <div>
      ...
      {editCell && (
        <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000 }} onClick={()=>setEditCell(null)}>
          <div className="card" style={{ padding:28,width:340 }} onClick={e=>e.stopPropagation()}>
