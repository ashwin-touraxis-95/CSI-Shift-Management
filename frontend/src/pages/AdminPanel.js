...
export default function AdminPanel() {
  ...
  const tabs = [
    ...
    { id:'bigquery',     label:'📊 BigQuery Sync' },
          ...
          <div className="card" style={{ padding:24, marginBottom:24 }}>
            ...
            <div style={{ display:'flex', gap:12, flexWrap:'wrap', alignItems:'flex-end' }}>
              <div style={{ minWidth:40 }}>
                <label>Icon</label>
                <input value={newBreak.icon} onChange={e=>setNewBreak(b=>({...b,icon:e.target.value}))} style={{ width:60,textAlign:'center',fontSize:20 }}/>
              </div>
                ...
                {(Array.isArray(breakTypes) ? breakTypes : []).map(bt=>(
                  <tr key={bt.id} style={{ borderBottom:'1px solid var(--gray-100)' }}>
                    {editingBreak?.id === bt.id ? (
                      <>
                        <td style={{ padding:'8px 16px' }}><input value={editingBreak.icon} onChange={e=>setEditingBreak(b=>({...b,icon:e.target.value}))} style={{ width:60,textAlign:'center',fontSize:20 }}/></td>
      ...
      {tab === 'bigquery' && (
        <div className="fade-in" style={{ maxWidth:600 }}>
          <div className="card" style={{ padding:32 }}>
            <h3 style={{ marginBottom:8 }}>📊 BigQuery Data Warehouse</h3>
            <p style={{ color:'var(--gray-500)', fontSize:13, marginBottom:24, lineHeight:1.7 }}>
              Sync all ShiftManager data to your Google BigQuery warehouse. Data syncs automatically every night at midnight SAST. Use the button below for a manual sync at any time.
            </p>
            <div style={{ background:'#F0FDF4', border:'1px solid #A7F3D0', borderRadius:10, padding:16, marginBottom:20, fontSize:13 }}>
              <div style={{ fontWeight:700, color:'#065F46', marginBottom:6 }}>Tables synced to BigQuery</div>
            ...
            <div style={{ background:'#EFF6FF', border:'1px solid #BFDBFE', borderRadius:10, padding:16, marginBottom:24, fontSize:13, color:'#1D4ED8', lineHeight:1.7 }}>
              ⏰ Auto-sync runs daily at <strong>00:00 SAST (22:00 UTC)</strong>
            </div>
            <button className="btn btn-primary" style={{ display:'flex', alignItems:'center', gap:8 }} onClick={async (e) => {
              const btn = e.target;
              btn.disabled = true;
              btn.textContent = 'Syncing...';
              try {
                const r = await axios.post('/api/admin/bigquery-sync');
                if (r.data.ok) {
                  btn.textContent = '✅ Sync complete in ' + r.data.duration + 's';
                } else {
                  const failed = Object.entries(r.data.tables||{}).filter(([,v])=>!v.ok).map(([k])=>k).join(', ');
                  btn.textContent = failed ? '⚠️ Partial — failed: ' + failed : '❌ Failed — check logs';
                }
                setTimeout(() => { btn.disabled=false; btn.textContent='🔄 Sync Now'; }, 5000);
              } catch(e) {
                btn.textContent = '❌ Error: ' + (e.response?.data?.error || e.message);
                setTimeout(() => { btn.disabled=false; btn.textContent='🔄 Sync Now'; }, 5000);
              }
            }}>
              🔄 Sync Now
