...
export default function AdminPanel() {
  ...
  const [scheduleViews, setScheduleViews] = useState({ manager:'week', team_leader:'week', agent:'week' });
  const [theme, setTheme] = useState({});
  const [departments, setDepartments] = useState([]);
  const [allUsers, setAllUsers] = useState([]);
  const [jobRoles, setJobRoles] = useState([]);
  const [auditLog, setAuditLog] = useState([]);
  const [expandedAuditUsers, setExpandedAuditUsers] = useState([]);
  ...
  const [newHoliday, setNewHoliday] = useState({ date:'', name:'', location:'SA' });
  ...
  const fetchAll = async () => {
    try {
      ...
      // Load schedule default views per role from theme settings
      try {
        const svr = await axios.get('/api/theme').catch(()=>({data:{}}));
        const sv = svr.data || {};
        setScheduleViews({
          manager:     sv.schedule_default_view_manager     || 'week',
          team_leader: sv.schedule_default_view_team_leader || 'week',
          agent:       sv.schedule_default_view_agent       || 'week',
        });
      } catch(e) {}
      setTheme(th.data);
  ...
  const addHoliday = async () => {
    ...
    try {
      ...
      setNewHoliday({ date:'', name:'', location:'SA' });
  ...
  const savePerms = async (role) => {
    ...
    // Save schedule default view for this role
    const key = `schedule_default_view_${role}`;
    await axios.put('/api/theme', { [key]: scheduleViews[role] || 'week' }).catch(()=>{});
    msg(`${USER_TYPE_LABELS[role]} permissions saved!`);
  };
  ...
  const tabs = [
    ...
    { id:'visibility',  label:'🔐 Permissions & Visibility', show: isAccountAdmin },
    ...
    { id:'audit',       label:'📋 User Audit Logs',     show: isAccountAdmin },
  ].filter(t => t.show);
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#1a1a2e,#C0392B)', borderRadius:14, padding:'22px 28px', marginBottom:16, color:'white', display:'flex', alignItems:'center', gap:14 }}>
      ...
      {saved && <div style={{ background:'#d4edda', border:'1px solid #c3e6cb', borderRadius:8, padding:'10px 16px', marginBottom:12, color:'#155724', fontSize:14 }}>✓ {saved}</div>}

      <div style={{ display:'flex', gap:4, borderBottom:'2px solid var(--gray-200)', overflowX:'auto' }}>
        {tabs.map(t => <button key={t.id} onClick={() => { setTab(t.id); if(t.id==='audit') fetchAudit(); }}
          style={{ padding:'10px 20px', border:'none', background:'none', cursor:'pointer', fontFamily:'inherit', fontSize:14, fontWeight:600, color:tab===t.id?'var(--red)':'var(--gray-500)', borderBottom:tab===t.id?'2px solid var(--red)':'2px solid transparent', marginBottom:-2, whiteSpace:'nowrap' }}>{t.label}</button>)}
      </div>
          ...
          <div className="fade-in">
            <div className="card" style={{ padding:28, marginBottom:20 }}>
              ...
              <div style={{ marginTop:20 }}>
                <div style={{ fontSize:12, fontWeight:700, color:'var(--gray-500)', textTransform:'uppercase', letterSpacing:0.8, marginBottom:10 }}>Daily Schedule Embed PIN</div>
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  <input
                    type="text"
                    inputMode="numeric"
                    maxLength={8}
                    value={theme.embed_pin || ''}
                    onChange={e => setTheme(t => ({ ...t, embed_pin: e.target.value.replace(/\D/g,'') }))}
                    placeholder="e.g. 5678"
                    style={{ width:160 }}
                  />
                  <button className="btn btn-primary" onClick={async () => { await axios.put('/api/theme', theme); updateTheme(theme); msg('Embed PIN saved!'); }}>Save PIN</button>
                </div>
                <p style={{ fontSize:11, color:'var(--gray-400)', marginTop:8 }}>
                  PIN for the embeddable daily schedule at <strong>/embed/daily</strong> — shows Sales &amp; Travel Agents shifts. Session persists until browser is closed.
                </p>
              </div>

              {/* Embed Daily URL */}
              <div style={{ background:'var(--gray-50,#F9FAFB)', border:'1px solid var(--gray-200)', borderRadius:10, padding:16, marginTop:20 }}>
                <div style={{ fontSize:12, fontWeight:700, color:'var(--gray-500)', textTransform:'uppercase', letterSpacing:0.8, marginBottom:8 }}>Daily Schedule Embed URL</div>
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  <code style={{ flex:1, fontSize:13, color:'var(--gray-700)', background:'white', border:'1px solid var(--gray-200)', borderRadius:8, padding:'8px 12px', wordBreak:'break-all' }}>{`${window.location.origin}/embed/daily`}</code>
                  <button className="btn btn-secondary" onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/embed/daily`); msg('Embed URL copied!'); }}>📋 Copy</button>
                  <a href="/embed/daily" target="_blank" rel="noreferrer" className="btn btn-secondary" style={{ textDecoration:'none' }}>🔗 Open</a>
                </div>
                <p style={{ fontSize:11, color:'var(--gray-400)', marginTop:8 }}>
                  Use this URL to embed the daily schedule in Google Sites or any iframe.
                </p>
              </div>
            </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:20 }}>
            {USER_TYPES.map(ut => (
              <div key={ut.key} className="card" style={{ padding:24, borderTop:`4px solid ${ut.color}` }}>
                ...
                <div style={{ marginTop:14, padding:'12px', borderRadius:8, background:'var(--gray-50)', border:'1px solid var(--gray-200)' }}>
                  <div style={{ fontSize:12, fontWeight:700, color:'var(--gray-700)', marginBottom:8 }}>📊 Schedule Default View</div>
                  <div style={{ display:'flex', gap:6 }}>
                    {[{v:'day',l:'Day'},{v:'week',l:'Week'},{v:'month',l:'Month'}].map(opt => {
                      const active = (scheduleViews[ut.key]||'week') === opt.v;
                      return (
                        <button key={opt.v}
                          onClick={()=>setScheduleViews(sv=>({...sv,[ut.key]:opt.v}))}
                          style={{ flex:1, padding:'6px 0', borderRadius:7, border:`1.5px solid ${active?ut.color:'var(--gray-200)'}`, background:active?`${ut.color}18`:'white', color:active?ut.color:'var(--gray-500)', fontFamily:'inherit', fontSize:12, fontWeight:active?700:500, cursor:'pointer', transition:'all 0.15s' }}>
                          {opt.l}
                        </button>
                      );
                    })}
                ...
                <button className="btn btn-sm" style={{ marginTop:10,width:'100%',justifyContent:'center',background:ut.color,color:'white',border:'none' }} onClick={()=>savePerms(ut.key)}>
      ...
      {/* Permissions section merged into visibility tab */}
      {tab === 'visibility' && (
        <div className="fade-in" style={{ marginTop:24 }}>
          <h3 style={{ fontWeight:700, fontSize:16, marginBottom:4 }}>🔐 Role Permissions</h3>
          <p style={{ color:'var(--gray-500)', fontSize:13, marginBottom:20 }}>Configure what each User Type can see and do. Changes take effect immediately.</p>
      ...
      {tab === 'audit' && (
        <div className="fade-in">
          <p style={{ color:'var(--gray-500)',fontSize:14,marginBottom:20 }}>Full record of all user account changes, grouped by user.</p>
          {(() => {
            const ACTION_LABELS_FULL = {
              user_created:    { label:'➕ Created',           color:'#16a34a', bg:'#f0fdf4' },
              user_deleted:    { label:'🗑 Deleted',           color:'#dc2626', bg:'#fef2f2' },
              user_activated:  { label:'✅ Activated',         color:'#16a34a', bg:'#f0fdf4' },
              user_deactivated:{ label:'⏸ Deactivated',       color:'#d97706', bg:'#fffbeb' },
              password_reset:  { label:'🔑 Password Reset',   color:'#7c3aed', bg:'#f5f3ff' },
              password_changed:{ label:'🔒 Password Changed', color:'#2563eb', bg:'#eff6ff' },
              role_changed:    { label:'🎭 Role Changed',      color:'#0891b2', bg:'#ecfeff' },
              profile_updated: { label:'✏️ Profile Updated',  color:'#64748b', bg:'#f8fafc' },
            };
            // Group by target user
            const grouped = {};
            auditLog.forEach(log => {
              const key = log.target_user_id || log.target_user_email || 'unknown';
              if (!grouped[key]) grouped[key] = { name: log.target_user_name, email: log.target_user_email, role: log.target_user_role, dept: log.target_user_department, events: [] };
              grouped[key].events.push(log);
            });
            const users = Object.entries(grouped);
            if (users.length === 0) return <div style={{ textAlign:'center', padding:40, color:'var(--gray-400)' }}>No audit events yet</div>;
            return users.map(([key, u]) => {
              const isOpen = expandedAuditUsers?.includes(key);
              return (
                <div key={key} className="card" style={{ marginBottom:10, overflow:'hidden' }}>
                  {/* User header row — clickable */}
                  <div onClick={() => setExpandedAuditUsers(prev => isOpen ? (prev||[]).filter(k=>k!==key) : [...(prev||[]), key])}
                    style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 20px', cursor:'pointer', userSelect:'none', background:'white' }}>
                    <div style={{ width:36,height:36,borderRadius:'50%',background:'var(--red)',color:'white',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,fontWeight:700,flexShrink:0 }}>
                      {(u.name||'?')[0].toUpperCase()}
                    </div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:700, fontSize:14 }}>{u.name||'Unknown'}</div>
                      <div style={{ fontSize:12, color:'var(--gray-400)' }}>{u.email} · {u.dept||'—'} · <span style={{ textTransform:'capitalize' }}>{(u.role||'').replace('_',' ')}</span></div>
                    </div>
                    <div style={{ background:'var(--gray-100)', borderRadius:20, padding:'3px 12px', fontSize:12, fontWeight:600, color:'var(--gray-600)' }}>
                      {u.events.length} event{u.events.length!==1?'s':''}
                    </div>
                    <div style={{ fontSize:18, color:'var(--gray-400)', transition:'transform 0.2s', transform: isOpen?'rotate(180deg)':'rotate(0deg)' }}>▾</div>
                  </div>
                  {/* Expanded events */}
                  {isOpen && (
                        ...
                        <thead>
                          <tr style={{ background:'var(--gray-50)' }}>
                            {['Action','Details','Performed By','Date & Time'].map(h=>(
                              <th key={h} style={{ padding:'10px 16px', textAlign:'left', fontSize:11, fontWeight:600, color:'var(--gray-400)', textTransform:'uppercase' }}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {u.events.map(log => {
                            const meta = ACTION_LABELS_FULL[log.action] || { label: log.action, color:'var(--gray-500)', bg:'white' };
                            return (
                              <tr key={log.id} style={{ borderTop:'1px solid var(--gray-100)' }}>
                                <td style={{ padding:'10px 16px' }}>
                                  <span style={{ background:meta.bg, color:meta.color, padding:'3px 10px', borderRadius:20, fontSize:12, fontWeight:600, whiteSpace:'nowrap' }}>{meta.label}</span>
                                </td>
                                <td style={{ padding:'10px 16px', fontSize:12, color:'var(--gray-500)', maxWidth:320 }}>{log.details||'—'}</td>
                                <td style={{ padding:'10px 16px', fontSize:13 }}>{log.performed_by_name||'—'}</td>
                                <td style={{ padding:'10px 16px', fontFamily:'DM Mono', fontSize:12, color:'var(--gray-400)', whiteSpace:'nowrap' }}>{log.created_at?new Date(log.created_at).toLocaleString('en-ZA'):'—'}</td>
                              </tr>
                            );
                          })}
                        </tbody>
          ...
          <div className="card" style={{ padding:32 }}>
            ...
            <div style={{ background:'#F0FDF4', border:'1px solid #A7F3D0', borderRadius:10, padding:14, marginBottom:16, fontSize:13 }}>
              ...
              <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
                {['users','shifts','leave','shift_templates','public_holidays','audit_log','clock_logs','break_logs','break_types','departments'].map(t => (
