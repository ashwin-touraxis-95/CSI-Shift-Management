...
export default function Layout({ children }) {
  ...
  const buildNav = () => {
    ...
    // Admin section
    const admin = [];
    if (isAdmin) admin.push({ to:'/admin', icon:'🛡️', label:'Admin Panel' });
    ...
    <div style={{ display:'flex', minHeight:'100vh' }}>
      <aside style={{ width:224, background:sidebarBg, color:'white', display:'flex', flexDirection:'column', position:'fixed', top:0, left:0, bottom:0, zIndex:100 }}>
        ...
        <nav style={{ padding:'12px 10px', flex:1, overflowY:'auto' }}>
          {(() => {
            ...
            const SectionLabel = ({ label }) => (
              <div style={{ fontSize:10,fontWeight:700,color:'rgba(255,255,255,0.25)',letterSpacing:1.5,padding:'14px 8px 8px',textTransform:'uppercase' }}>{label}</div>
            );
            return <>
        ...
        <div style={{ padding:14, borderTop:'1px solid rgba(255,255,255,0.07)' }}>
          ...
          <button onClick={()=>navigate('/change-password')} style={{ width:'100%',padding:'7px',borderRadius:8,border:'1px solid rgba(255,255,255,0.12)',background:'transparent',color:'rgba(255,255,255,0.4)',fontSize:12,cursor:'pointer',fontFamily:'inherit',marginBottom:6 }}>🔐 Change Password</button>
          ...
          <div style={{ marginTop:14, paddingTop:12, borderTop:'1px solid rgba(255,255,255,0.06)', textAlign:'center' }}>
            {theme?.footer_line1 && <div style={{ fontSize:10, color:'rgba(255,255,255,0.2)', lineHeight:1.8 }}>{theme.footer_line1}</div>}
            <div style={{ fontSize:10, color:'rgba(255,255,255,0.15)', lineHeight:1.6, display:'flex', alignItems:'center', justifyContent:'center', gap:5, flexWrap:'wrap' }}>
