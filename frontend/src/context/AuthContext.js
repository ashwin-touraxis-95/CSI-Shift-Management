...
// Global interceptor — handle session expired (signed in on another device)
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    const handler = () => { setUser(null); setPermissions(null); setSessionExpired(true); };
  ...
  const applyTheme = (t) => {
    ...
    if (t.tab_title) document.title = t.tab_title;
    else if (t.company_name) document.title = t.company_name;
  };
  ...
  return (
    <AuthContext.Provider value={{ user, permissions, theme, loading, login, logout, can, isAdmin, isManager, isLeader, isPH, needsOnboarding, updateTheme, updateUser, sessionExpired, clearSessionExpired: () => setSessionExpired(false) }}>
      {children}
