...
const DEFAULT_PERMISSIONS = {
  manager: {
    view_availability:true, view_all_departments:true, manage_shifts:true, publish_shifts:true,
    view_clock_logs:true, view_own_logs_only:false, manage_users:true, manage_settings:false,
    view_drafts:true, assign_team_leaders:true, show_shifts_this_month:true, show_total_hours:true, can_set_active_status:true,
  },
  team_leader: {
    view_availability:true, view_all_departments:true, manage_shifts:true, publish_shifts:false,
    view_clock_logs:true, view_own_logs_only:true, manage_users:true, manage_settings:false,
    view_drafts:true, assign_team_leaders:false, show_shifts_this_month:true, show_total_hours:true, can_set_active_status:true,
  },
  agent: {
    view_availability:true, view_all_departments:true, manage_shifts:false, publish_shifts:false,
    view_clock_logs:false, view_own_logs_only:true, manage_users:false, manage_settings:false,
    view_drafts:false, assign_team_leaders:false, show_shifts_this_month:false, show_total_hours:false, can_set_active_status:false,
  }
};
...
const DEFAULT_THEME = {
  ...
  company_name:'ShiftManager', location_label:'South Africa', company_logo:null,
};
...
async function initDb() {
  ...
  db.run(`CREATE TABLE IF NOT EXISTS users (
    ...
    password_hash TEXT, force_password_change INTEGER DEFAULT 0,
    created_at TEXT DEFAULT (datetime('now'))
  )`);
  db.run(`CREATE TABLE IF NOT EXISTS departments (
    id TEXT PRIMARY KEY, name TEXT NOT NULL, color TEXT DEFAULT '#333333', bg_color TEXT DEFAULT '#F0F0F0', active INTEGER DEFAULT 1
  )`);
  db.run(`CREATE TABLE IF NOT EXISTS department_managers (
    department_id TEXT NOT NULL, manager_id TEXT NOT NULL, PRIMARY KEY (department_id, manager_id)
  )`);
  ...
  db.run(`CREATE TABLE IF NOT EXISTS job_role_leaders (
    job_role_id TEXT NOT NULL, leader_id TEXT NOT NULL, PRIMARY KEY (job_role_id, leader_id)
  )`);
  db.run(`CREATE TABLE IF NOT EXISTS agent_job_roles (
    agent_id TEXT NOT NULL, job_role_id TEXT NOT NULL, PRIMARY KEY (agent_id, job_role_id)
  )`);
  ...
  db.run(`CREATE TABLE IF NOT EXISTS theme (key TEXT PRIMARY KEY, value TEXT NOT NULL)`);
  db.run(`CREATE TABLE IF NOT EXISTS role_permissions (role TEXT PRIMARY KEY, permissions TEXT NOT NULL)`);
  db.run(`CREATE TABLE IF NOT EXISTS team_leader_agents (leader_id TEXT NOT NULL, agent_id TEXT NOT NULL, PRIMARY KEY (leader_id, agent_id))`);
  ...
  ]) { try { db.run(col); saveDb(); } catch(e) {} }

  // Migrate old role column to user_type if needed
  try { db.run('UPDATE users SET user_type = role WHERE (user_type IS NULL OR user_type = "") AND role IS NOT NULL'); saveDb(); } catch(e) {}

  // Seed admin accounts — no password set yet (will be prompted on first login)
  for (const [email, name, type, dept] of [
    ...
    if (!get('SELECT id FROM users WHERE email=?', [email])) {
      const id = uuidv4();
      run('INSERT INTO users(id,email,name,user_type,department,active,onboarded) VALUES(?,?,?,?,?,1,1)', [id,email,name,type,dept]);
