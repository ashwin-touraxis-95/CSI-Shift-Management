...
async function initDb() {
  ...
  await query(`CREATE TABLE IF NOT EXISTS leave_types (
    ...
    active INTEGER DEFAULT 1, sort_order INTEGER DEFAULT 0, is_ph_off INTEGER DEFAULT 0
  )`);
  ...
  try { await query("ALTER TABLE leave_types ADD COLUMN is_ph_off INTEGER DEFAULT 0"); } catch(e) {}
  ...
  // Add color column to shifts and shift_templates
  try { await query("ALTER TABLE shifts ADD COLUMN color TEXT DEFAULT NULL"); } catch(e) {}
  try { await query("ALTER TABLE shift_templates ADD COLUMN color TEXT DEFAULT NULL"); } catch(e) {}
  try { await query("ALTER TABLE shifts ADD COLUMN text_color TEXT DEFAULT NULL"); } catch(e) {}
  try { await query("ALTER TABLE shift_templates ADD COLUMN text_color TEXT DEFAULT NULL"); } catch(e) {}
  // Sync colors on all shifts by matching start_time+end_time to a template
  try {
    await query(`
      UPDATE shifts
      SET color = t.color, text_color = t.text_color
      FROM shift_templates t
      WHERE shifts.start_time = t.start_time
        AND shifts.end_time = t.end_time
        AND t.color IS NOT NULL
    `);
  } catch(e) { console.error('shift color backfill error:', e.message); }
  ...
  const defaultLeaveTypes = [
    ...
    { name:'PH - Off',                   color:'#b45309', bg_color:'#fef3c7', half_day_allowed:0, paid_hours:8, is_ph_off:1 },
  ];
  for (let i = 0; i < defaultLeaveTypes.length; i++) {
    ...
    if (!existing) await run('INSERT INTO leave_types(id,name,color,bg_color,half_day_allowed,paid_hours,is_ph_off,sort_order,active) VALUES($1,$2,$3,$4,$5,$6,$7,$8,1)',
      [uuidv4(), lt.name, lt.color, lt.bg_color, lt.half_day_allowed, lt.paid_hours, lt.is_ph_off||0, i]);
  }
