...
async function runSync() {
  ...
  for (const t of tables) {
    try {
      ...
      const isBillingError = e.message && (e.message.includes('Billing') || e.message.includes('free tier') || e.message.includes('DML'));
      if (isBillingError) {
        console.error(`  ⚠️  ${t.name}: BigQuery billing required`);
        results[t.name] = { ok: false, error: 'billing_required' };
      } else {
