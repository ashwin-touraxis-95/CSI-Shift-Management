...
app.get('/api/shifts', requireAuth, async (req, res) => {
  ...
  if (u.user_type==='agent') {
    ...
    if (ids.length) {
      const placeholders = ids.map(() => { p.push(ids[p.length]); return `$${p.length}`; });
      // rebuild - push ids to p first then build placeholders
      p.length = 0;
...
initDb().then(() => {
  server.listen(PORT, () => {
    console.log('✅ ShiftManager v10 running on port', PORT);
