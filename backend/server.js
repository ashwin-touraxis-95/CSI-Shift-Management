...
const { runSync } = require('./bigquery-sync');
...
initDb().then(() => {
  server.listen(PORT, () => {
    console.log('✅ ShiftManager v9 running on port', PORT);
    if (!isProduction) console.log('   Frontend: http://localhost:3000');
  });
  
  // Schedule BigQuery sync daily at midnight SA time (UTC+2 = 22:00 UTC)
  if (isProduction && process.env.GOOGLE_SERVICE_ACCOUNT_KEY) {
    cron.schedule('0 22 * * *', async () => {
      console.log('⏰ Running scheduled BigQuery sync...');
      await runSync();
    }, { timezone: 'UTC' });
    console.log('📅 BigQuery daily sync scheduled for 00:00 SAST');
    
    // Also run sync on startup after 30 seconds
    setTimeout(async () => {
      console.log('🔄 Running initial BigQuery sync...');
      await runSync();
    }, 30000);
  }
