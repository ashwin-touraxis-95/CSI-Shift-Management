...
app.use(session({
  ...
  cookie: { secure: isProduction, maxAge: 24*60*60*1000, sameSite: isProduction ? 'none' : 'lax' }
}));
...
app.post('/api/clock/in', requireAuth, async (req, res) => {
  ...
  await run("INSERT INTO clock_logs(id,user_id,clock_in,date,ip_address) VALUES($1,$2,NOW(),$1,$2)",[uuidv4(),req.session.userId,today,req.ip]);
...
app.post('/api/onboarding/complete', requireAuth, async (req, res) => {
  ...
  if (!job_role_ids$1.length) return res.status(400).json({ error:'Select at least one job role' });
...
app.get('/api/shifts', requireAuth, async (req, res) => {
  ...
  const p = [];
  if (u.user_type==='agent') { q+=` AND s.user_id=$1 AND s.status='published'`; p.push(u.id); }
  else if (u.user_type==='team_leader') {
    const ids = await all('SELECT agent_id FROM team_leader_agents WHERE leader_id=$1',[u.id]).map(r=>r.agent_id);
    if (ids.length) { q+=` AND s.user_id IN (${ids.map(()=>'$1').join(',')}) AND s.status='published'`; p.push(...ids); }
    else q+=` AND 1=0`;
  }
  if (user_id && u.user_type!=='agent') { q+=` AND s.user_id=$1`; p.push(user_id); }
  if (start && end) { q+=` AND s.date>=$2 AND s.date<=$3`; p.push(start,end); }
  if (req.query.status && ['account_admin','manager'].includes(u.user_type)) { q+=` AND s.status=$1`; p.push(req.query.status); }
...
app.post('/api/shifts', requireAuth, requirePerm('manage_shifts'), async (req, res) => {
  ...
  const department = agent$1.department || 'CS';
  const canPublish = req.user.user_type==='account_admin'||req.perms$1.publish_shifts;
  const id = uuidv4();
  await run('INSERT INTO shifts(id,user_id,date,start_time,end_time,department,notes,status,created_by) VALUES($2,$3,$4,$5,$6,$7,$8,$9,$10)',
...
app.post('/api/shifts/bulk', requireAuth, requirePerm('manage_shifts'), async (req, res) => {
  ...
  for (const uid of user_ids) {
    const agent = await get('SELECT department FROM users WHERE id=$11', [uid]);
    const department = agent$1.department || 'CS';
...
app.put('/api/shifts/:id', requireAuth, requirePerm('manage_shifts'), async (req, res) => {
  ...
  const canPublish = req.user.user_type==='account_admin'||req.perms$1.publish_shifts;
  await run('UPDATE shifts SET date=$13,start_time=$14,end_time=$15,department=$16,notes=$17,status=$18 WHERE id=$19',
...
app.post('/api/templates', requireAuth, requirePerm('manage_shifts'), async (req, res) => {
  ...
  const id = uuidv4();
  await run('INSERT INTO shift_templates(id,name,start_time,end_time,department,notes,created_by) VALUES($1,$2,$3,$4,$5,$6,$7)',[id,name,start_time,end_time,department,notes,req.session.userId]);
  res.json({ id, ok: true });
});
...
app.put('/api/users/:id', requireAuth, canManageUsers, async (req, res) => {
  ...
  await run('UPDATE users SET user_type=$20,department=$21,name=$22,active=$23 WHERE id=$24',
    [user_type!==undefined?user_type:target.user_type, department||target.department, name||target.name, finalActive, req.params.id]);
...
app.get('/api/logs', requireAuth, requirePerm('view_clock_logs'), async (req, res) => {
  ...
  const p = [];
  if (req.user.user_type==='team_leader'&&req.perms$1.view_own_logs_only) {
...
initDb().then(() => server.listen(PORT, () => {
  console.log('✅ ShiftManager v8 running on port', PORT);
  if (!isProduction) console.log('   Frontend: http://localhost:3000');
})).catch(err => { console.error('❌ Failed to start:', err); process.exit(1); });

// ── BREAK TYPES ───────────────────────────────────────────────────────────────
app.get('/api/break-types', requireAuth, async (req, res) => {
  res.json(await all('SELECT * FROM break_types WHERE active=1 ORDER BY sort_order,name'));
});

app.post('/api/break-types', requireAuth, async (req, res) => {
  const u = req.user;
  if (!['account_admin','manager'].includes(u.user_type)) return res.status(403).json({ error: 'Permission denied' });
  const { name, icon, color, max_minutes } = req.body;
  if (!name) return res.status(400).json({ error: 'Name required' });
  const countRow = await get('SELECT COUNT(*) as c FROM break_types'); const count = countRow?.c || 0;
  const id = uuidv4();
  await run('INSERT INTO break_types(id,name,icon,color,max_minutes,sort_order) VALUES($1,$2,$3,$4,$5,$6)',
...
app.delete('/api/break-types/:id', requireAuth, async (req, res) => {
  const u = req.user;
  if (!['account_admin','manager'].includes(u.user_type)) return res.status(403).json({ error: 'Permission denied' });
  await run('UPDATE break_types SET active=0 WHERE id=$1', [req.params.id]);
  res.json({ ok: true });
});
...
app.post('/api/breaks/start', requireAuth, async (req, res) => {
  ...
  await run(`UPDATE availability SET status='on_break', break_type_id=$1, break_type_name=$2, break_type_icon=$3, break_type_color=$4, last_updated=NOW() WHERE user_id=$1`,
...
app.get('/api/breaks/status', requireAuth, async (req, res) => {
  ...
  const todayBreaks = await all("SELECT * FROM break_logs WHERE user_id=$3 AND date=$4 ORDER BY started_at", [req.session.userId, today]);
...
app.post('/api/override/clock-in', requireAuth, async (req, res) => {
  ...
  await run("INSERT INTO clock_logs(id,user_id,clock_in,date,ip_address) VALUES($1,$2,NOW(),$1,$2)", [require('uuid').v4(), user_id, today, 'manager-override']);
...
app.post('/api/override/clock-out', requireAuth, async (req, res) => {
  ...
  const open = await get("SELECT id FROM clock_logs WHERE user_id=$3 AND date=$4 AND clock_out IS NULL", [user_id, today]);
...
app.post('/api/override/start-break', requireAuth, async (req, res) => {
  ...
  const bt = await get('SELECT * FROM break_types WHERE id=?', [break_type_id]);
  if (!bt) return res.status(404).json({ error: 'Break type not found' });
  await run("INSERT INTO break_logs(id,user_id,break_type_id,break_type_name,break_type_icon,break_type_color,started_at,date) VALUES($1,$2,$3,$4,$5,$6,NOW(),$1)",
    [require('uuid').v4(), user_id, bt.id, bt.name, bt.icon, bt.color, today]);
  await run("UPDATE availability SET status='on_break',break_type_id=$1,break_type_name=$2,break_type_icon=$3,break_type_color=$4,last_updated=NOW() WHERE user_id=$1",
